...
class TestOpenAIEndToEnd:
    ...
    def test_openai_api_key_available(self):
        ...
        if not api_key.startswith("sk-"):
            pytest.skip("OPENAI_API_KEY appears to be a placeholder or invalid format")

    @pytest.mark.skipif(
        not os.getenv("OPENAI_API_KEY")
        or not os.getenv("OPENAI_API_KEY", "").startswith("sk-"),
        reason="OpenAI API key not available or invalid format",
    )
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "Say 'Hello LlamaFarm!' and nothing else.",
                    }
                ],
                max_tokens=10,
                temperature=0,
            ...
            if "organization" in str(e).lower():
                pytest.skip(
                    "OpenAI organization configuration issue - test environment specific"
                )
            else:
...
class TestOllamaEndToEnd:
    ...
    def test_ollama_installed(self):
        ...
        try:
            result = subprocess.run(
                ["ollama", "--version"], capture_output=True, text=True, timeout=30
            )
    ...
    def test_ollama_api_connectivity(self):
        ...
        try:
            ...
            assert response.status_code == 200, (
                f"Ollama API not responding: {response.status_code}"
            )
    ...
    @pytest.mark.skipif(
        not os.system("which ollama > /dev/null 2>&1") == 0,
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                if "model" in error_text.lower() and (
                    "not found" in error_text.lower() or "load" in error_text.lower()
                ):
            ...
            if not data["response"]:
                # Skip instead of failing - this might be a model-specific issue
                pytest.skip(
                    f"Model {model_name} returned empty response - may be a model configuration issue"
                )

            # Basic validation that we got something resembling expected output
            assert len(data["response"].strip()) > 0, "Response is only whitespace"
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if lines and lines[0].strip():
                    ...
                    # Test the model
                    args = Namespace(model=model_name, query="Hello")
    ...
    def test_openai_response_time(self):
        ...
        try:
            ...
            assert response_time < 10.0, (
                f"OpenAI API response too slow: {response_time:.2f}s"
            )
            assert response.choices[0].message.content is not None
    ...
    def test_ollama_response_time(self):
        ...
        try:
            ...
            assert response_time < 30.0, (
                f"Ollama response too slow: {response_time:.2f}s"
            )
            assert response.status_code == 200, (
