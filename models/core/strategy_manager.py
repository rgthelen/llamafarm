...
class StrategyManager:
    ...
    def __init__(self, strategies_file: Optional[Path] = None):
        ...
        if strategies_file:
            ...
            self.strategies_file = (
                Path(__file__).parent.parent / "default_strategies.yaml"
            )
    ...
    def build_component_config(
        self, strategy_name: str, component_type: str
    ...
    def validate_strategy(self, strategy_name: str) -> List[str]:
        ...
        for comp_type, comp_config in components.items():
            ...
            if not isinstance(comp_config, dict):
                errors.append(
                    f"Component {comp_type} configuration must be a dictionary"
                )
    ...
    def _expand_env_vars(
        self, config: Union[Dict, List, str, Any]
    ...
    def merge_strategies(
        self, base_strategy: str, overrides: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def export_strategy(
        self, strategy_name: str, output_path: Path, format: str = "yaml"
    ) -> bool:
