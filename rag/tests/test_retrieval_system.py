...
def test_basic_setup():
    ...
    # Test factory
    strategies = RetrievalStrategyFactory.list_available()
    print(f"✅ Found {len(strategies)} available strategies:")
    for name in strategies[:5]:  # Show first 5
        print(f"   - {name}")
    
    # Test strategy creation
    try:
        basic_strategy = RetrievalStrategyFactory.create("BasicSimilarityStrategy")
        print(f"✅ Successfully created basic strategy")
